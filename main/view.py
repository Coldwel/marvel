    ...
    def display_comics_list(self, comics_list):
        ...
        for comic_item in comics_list:
            ...
            if thumbnail_url:
                ...
                image_data = io.BytesIO(image_bytes)
                img = Image.open(image_data)
                img = img.resize((150, 200))  # Adjust the size as needed
                img = ImageTk.PhotoImage(img)
                ...
                thumbnail_label.grid(row=comics_list.index(comic_item), column=0, padx=5, pady=5, sticky=tk.W)

            # Display the comic title
            title_label = tk.Label(comics_frame, text=comic_item['title'], bg=BG_COLOR, fg=TEXT_COLOR, font=LABEL_FONT)
            title_label.grid(row=comics_list.index(comic_item), column=1, padx=5, pady=5, sticky=tk.W)

        # Add a vertical scrollbar for the Canvas
        scrollbar = tk.Scrollbar(comics_window, orient=tk.VERTICAL, command=comics_canvas.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
